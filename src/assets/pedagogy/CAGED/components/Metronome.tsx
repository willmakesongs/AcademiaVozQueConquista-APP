    ...
    <div className="w-full max-w-2xl bg-[#0e1017] p-8 md:p-16 rounded-[4rem] border border-white/5 shadow-2xl flex flex-col items-center gap-10 relative overflow-hidden animate-in fade-in duration-500">
      ...
      <div className="flex gap-4">
        {[0, 1, 2, 3].map(i => (
          <div
            key={i}
            className={`w-4 h-4 rounded-full transition-all duration-75 ${isPlaying && currentBeatVisual === i
      ...
      <div className="flex items-center gap-4 bg-black/20 px-6 py-4 rounded-3xl border border-white/5 w-full max-w-xs">
        ...
        <label htmlFor="volume-caged" className="sr-only">Volume do Metrônomo</label>
        <input
          id="volume-caged"
      ...
      <div className="flex flex-wrap justify-center gap-2 bg-black/30 p-2 rounded-2xl border border-white/5">
        {sounds.map(s => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${soundType === s.id
      ...
      <div className="w-full space-y-8">
        <div className="flex items-center gap-6 justify-center">
          ...
          <label htmlFor="bpm-caged" className="sr-only">Ajuste de BPM</label>
          <input
            id="bpm-caged"
        ...
        <div className="flex flex-col md:flex-row justify-center items-center gap-4">
          <button
            ...
            className={`group relative flex items-center gap-4 px-12 py-6 rounded-3xl font-black text-xl tracking-tighter transition-all active:scale-95 ${isPlaying
