        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-4 bg-[#0e1017] p-8 rounded-[3rem] border border-white/5 shadow-xl">
            <h3 className="text-white font-black text-[10px] uppercase tracking-[0.4em] mb-6 flex items-center gap-2">
              <Hash size={14} className="text-[#0081FF]" /> Escolha a Tônica
            </h3>
            <div className="grid grid-cols-4 gap-3">
              {NOTES.map(note => (
                <button
                  ...
                  aria-label={`Escolher tônica ${note}`}
                  className={`py-4 rounded-2xl font-black text-xs transition-all ${selectedRoot === note ? 'bg-blue-600 text-white shadow-lg shadow-[#0081FF]/20 scale-105' : 'bg-white/5 text-slate-500 hover:bg-white/10'
                    }`}
                >
                  {note}
                </button>
          ...
          <div className="lg:col-span-8 bg-[#0e1017] p-8 rounded-[3rem] border border-white/5 shadow-xl flex flex-col justify-center">
            <h3 className="text-white font-black text-[10px] uppercase tracking-[0.4em] mb-4 flex items-center gap-2">
              <Settings2 size={14} className="text-[#0081FF]" /> Qualidade Harmônica
            </h3>
            <button
              onClick={() => setIsModalOpen(true)}
              aria-label="Alterar Qualidade Harmônica"
              className="group w-full p-8 bg-blue-600/5 border border-[#0081FF]/20 rounded-[2.5rem] flex items-center justify-between hover:bg-blue-600/10 transition-all active:scale-[0.98]"
            >
              <div className="flex flex-col items-start text-left">
                <span className="text-4xl md:text-5xl font-black text-white tracking-tighter">
                  {getDisplayName()}
                </span>
                <span className="text-[10px] font-black text-[#0081FF] uppercase tracking-widest mt-1">Clique para mudar o tipo de acorde</span>
              </div>
              ...
              <div className="flex gap-2">
                {cagedShapes.map(shape => (
                  <button
                    ...
                    className={`w-14 h-14 rounded-2xl text-sm font-black transition-all ${activeShape === shape ? 'bg-blue-600 text-white shadow-lg scale-110' : 'bg-white/5 text-slate-600 hover:text-white hover:bg-white/10'
                      }`}
                  >
                    {shape}
                  </button>
          ...
          <div className="p-10 md:p-24 flex flex-col items-center justify-center bg-[#08090d]">
            ...
            <div className="flex flex-row w-full max-w-md mx-auto gap-3 mb-12 px-2 md:px-0">
              <button
                ...
                className={`group flex-1 flex items-center justify-center gap-2 md:gap-3 px-4 py-4 md:px-8 md:py-5 rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest transition-all active:scale-95 whitespace-nowrap ${isAnimating ? 'bg-blue-400 text-white scale-95' : 'bg-blue-600 text-white hover:bg-[#0081FF] shadow-xl shadow-blue-600/20'
                  }`}
              >
                ...
                <div className="flex w-full gap-2 animate-in zoom-in-95">
                  ...
                  {hasCustomVersion && (
                    <button
                      ...
                      className="flex items-center justify-center w-12 h-auto md:w-16 rounded-2xl bg-[#FF00BC]/20 border border-[#FF00BC]/20 text-[#FF00BC] hover:bg-[#FF00BC] hover:text-white transition-all active:scale-95"
