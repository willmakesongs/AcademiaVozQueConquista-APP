...
export const TeacherDashboard: React.FC<Props> = ({ onNavigate, onLogout }) => {
    ...
    return (
        <div className="min-h-screen bg-[#101622] pb-24 relative">
            {/* Header */}
            <header className="pt-8 pb-6 px-6 bg-gradient-to-b from-[#1A202C] to-[#101622] border-b border-white/5">
                <div className="flex justify-between items-center mb-6 relative">
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            <img src={user?.avatarUrl || 'https://picsum.photos/200'} alt="Profile" className="w-12 h-12 rounded-full border-2 border-[#0081FF]" />
                            <div className="absolute -bottom-1 -right-1 bg-[#0081FF] text-white text-[10px] px-1.5 py-0.5 rounded-full border border-[#101622] font-bold">PRO</div>
                        </div>
                        <div>
                            <p className="text-xs text-gray-400 font-medium">Bem-vinda,</p>
                            <h2 className="text-lg font-bold text-white">{user?.name || 'Professora'}</h2>
                        </div>
                    </div>

                    <div ref={configRef} className="relative">
                        <button onClick={() => setShowConfig(!showConfig)} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-white hover:bg-white/10">
                            <span className="material-symbols-rounded">settings</span>
                        </button>
                        {showConfig && (
                            <div className="absolute right-0 top-12 w-48 bg-[#1A202C] rounded-xl border border-white/10 shadow-xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                                <div className="p-1 bg-red-500/5">
                                    <button onClick={onLogout} className="w-full text-left px-3 py-2 rounded-lg hover:bg-red-500/10 text-sm flex items-center gap-2 text-red-500 font-medium">
                ...
                {/* Tabs */}
                {user?.role === 'admin' && (
                    <div className="flex gap-2 p-1 bg-[#101622] rounded-xl border border-white/5 mt-4">
                        <button
                            onClick={() => setActiveTab('students')}
                            className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'students' ? 'bg-[#0081FF] text-white shadow-lg' : 'text-gray-500 hover:text-white'}`}
                        >
                            Lista de Alunos
                        </button>
                        <button
                            onClick={() => setActiveTab('dashboard')}
                            className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'dashboard' ? 'bg-[#0081FF] text-white shadow-lg' : 'text-gray-500 hover:text-white'}`}
                        >
                            Estatísticas
                        </button>
                    </div>
                ...
                {activeTab === 'dashboard' && user?.role === 'admin' ? (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom duration-300">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            <div className="bg-gradient-to-br from-[#1A202C] to-[#101622] p-6 rounded-2xl border border-white/5 shadow-xl">
                                <p className="text-xs text-gray-400 uppercase font-bold tracking-widest mb-2">Faturamento Estimado</p>
                                <div className="flex items-baseline gap-2">
                                    <span className="text-3xl font-black text-white">R$ {students.reduce((acc, s) => acc + (s.amount || 0), 0).toLocaleString('pt-BR')}</span>
                                    <span className="text-xs text-green-400 font-bold">/ mês</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-[#1A202C] p-4 rounded-2xl border border-white/5">
                                <p className="text-[10px] text-gray-500 font-bold uppercase mb-1">Pagos</p>
                                <p className="text-xl font-bold text-green-400">{students.filter(s => s.status === 'active').length}</p>
                            </div>
                            <div className="bg-[#1A202C] p-4 rounded-2xl border border-white/5">
                                <p className="text-[10px] text-gray-500 font-bold uppercase mb-1">Pendentes</p>
                                <p className="text-xl font-bold text-amber-500">{students.filter(s => s.status === 'overdue').length}</p>
                            </div>
                        </div>

                        {/* Recent Activity / Quick Logs */}
                        <div className="space-y-3">
                            <h4 className="text-sm font-bold text-white uppercase tracking-wider ml-1">Alertas Fiscais</h4>
                            {students.filter(s => s.status === 'overdue' || s.status === 'blocked').map(s => (
                                <div key={s.id} className="p-3 bg-red-500/5 border border-red-500/10 rounded-xl flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-2 h-2 rounded-full ${s.status === 'blocked' ? 'bg-red-500' : 'bg-amber-500'}`}></div>
                                        <p className="text-sm text-white font-medium">{s.name}</p>
                                    </div>
                                    <button
                                        onClick={() => openStudentDetails(s)}
                                        className="text-[10px] font-bold text-[#0081FF] hover:underline"
                                    >
                                        VER DETALHES
                                    </button>
                                </div>
